...
class PGAgent(BaseAgent):
    ...
    def estimate_advantage(self, obs: np.ndarray, rews_list: np.ndarray, q_values: np.ndarray, terminals: np.ndarray):
        ...
        if self.nn_baseline:
            ...
            values = normalize(values_unnormalized, np.mean(values_unnormalized), np.std(
                values_unnormalized)) * np.std(q_values) + np.mean(q_values)
    ...
    def _discounted_return(self, rewards):
        ...
        discounted_gamma_list = self.gamma ** np.arange(len(rewards))
        list_of_discounted_returns = [np.sum(rewards* discounted_gamma_list)] * len(rewards)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        """
        discounted_gamma_list = self.gamma ** np.arange(len(rewards))
        list_of_discounted_cumsums = [np.sum(rewards[i:]* discounted_gamma_list[:len(rewards)-i]) for i in range(len(rewards))]
